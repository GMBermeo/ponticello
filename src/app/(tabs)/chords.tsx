...
import { useTheme, FACE, RADIUS } from '@theme';
...
export default function ChordLibraryScreen() {
  ...
  return <Screen scroll={false} padded={false}>
    <TabHeader title="Chords" subtitle="Shapes for the cello’s own tuning. Square = root." />
    <FlatList ref={list} key={columns} numColumns={columns} data={rows} keyExtractor={(row) => row.id}
      ...
      ListHeaderComponent={<Stack gap={14} padX={12}>
        ...
        <Toggle label="All positions" hint="Mark every place the chord's notes fall, up to the octave — for improvising over it." value={allPositions} onChange={setAllPositions} />
        <TextInput value={query} onChangeText={setQuery} accessibilityLabel="Search chord library" placeholder="Find a chord or type" placeholderTextColor={theme.chrome.dim}
          autoCapitalize="none" style={{ backgroundColor: theme.chrome.fill, borderRadius: RADIUS.pill, paddingHorizontal: theme.s(18), minHeight: theme.tap, ...FACE.regular, fontSize: theme.font(16), color: theme.chrome.ink }} />
        {study ? <View style={{ padding: theme.s(16), backgroundColor: theme.chrome.surface, ...theme.corners(RADIUS.lg), gap: theme.s(10) }}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: theme.s(18) }}>
            <CelloChordDiagram chord={study} width={150} presentation="atlas" tapeColors={tapeColors} allPositions={allPositions} mode={arpeggio ? 'arpeggio' : 'voicing'} voicingIndex={Math.min(variant, Math.max(0, study.voicings.length - 1))} />
      ...
      renderItem={({ item }) => <ChordTile entry={item} tapeColors={tapeColors} allPositions={allPositions} selected={active?.id === item.id} onSelect={selectChord} />}
...
type ChordTileProps = { entry: ScaleChord; selected: boolean; tapeColors: boolean; allPositions: boolean; onSelect: (id: string) => void };

function ChordTile({ entry, selected, tapeColors, allPositions, onSelect }: ChordTileProps) {
  const theme = useTheme();
  ...
  return <View style={{ flex: 1, maxWidth: theme.scale.compact ? '50%' : '25%', paddingHorizontal: theme.s(4), marginTop: theme.s(16) }}>
    <PressableRow accessibilityLabel={`Explore ${study.symbol}${degreeSuffix}`} selected={selected} onPress={() => onSelect(entry.id)}
      style={{
        alignItems: 'center', ...theme.corners(RADIUS.lg), paddingVertical: theme.s(8), gap: theme.s(4),
        borderWidth: theme.rule(2),
        borderColor: selected ? theme.chrome.accent : 'transparent',
        backgroundColor: theme.chrome.surface,
      }}>
      <View ref={ref} onLayout={onLayout} style={{ alignSelf: 'stretch', alignItems: 'center' }}>
        <CelloChordDiagram chord={study} presentation="atlas" tapeColors={tapeColors} allPositions={allPositions} width={150} measuredWidth={size.width > 0 ? Math.min(size.width, theme.s(150)) : undefined} />
      </View>
