...
import { ArrangementLevel, arrangeScoreForLevel } from './arrangement';
import { BackingPart, BackingTrack } from './backing';
import { RawNoteEvent } from './fingering';
import { CelloSongScore } from './schema';
import {
  DEFAULT_TRACK_CHOICE, findCelloPart, OctaveFit, partLabel, scoreFromPart,
  TrackChoice,
} from './trackPicker';
import {
...
export function applyChoice(
  ...
  if (!built) {
    const octaves = Math.abs(choice.octaves);
    return {
      score: arranged,
      backing,
      line: {
        ...line,
        fellBackBecause: `${partLabel(part)} ${octaves === 0 ? 'at written pitch'
          : `${octaves} octave${octaves === 1 ? '' : 's'} ${choice.octaves < 0 ? 'down' : 'up'}`}`
