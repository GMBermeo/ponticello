...
import {
  ARRANGEMENT_LEVELS, ARRANGEMENT_PROFILES, arrangeScoreForLevel, soloPartFromScore,
  midiToFrequency, midiToPitchName, OPEN_STRING_MIDI, difficultyOf, seatLine, scoreDurationMs,
  validateScore, type ArrangementLevel, type BackingTrack, type CelloNote, type CelloSongScore,
} from '@domain';
import { LIBRARY_ROWS, SCORES } from '@scores';
import { LIBRARY_EDITION } from '../libraryEdition';
...
/** Beginner plays few attacks per second, inside the neck, with no leap wider than its profile allows. */
function expectBeginnerLine(id: string, beginner: CelloSongScore): void {
  const seconds = Math.max(1, scoreDurationMs(beginner) / 1000);
  expect(beginner.notes.length / seconds, `${id} attacks/s`)
  ...
  expect(
    ...
    `${id} beginner stays in the neck`,
  ).toBe(true);
  for (let i = 1; i < beginner.notes.length; i++) {
    expect(
      Math.abs(beginner.notes[i]!.midiNumber - beginner.notes[i - 1]!.midiNumber),
      `${id} beginner leap at note ${i}`,
    ).toBeLessThanOrEqual(ARRANGEMENT_PROFILES.Beginner.maxLeapSemitones);
  }
}

/**
 * Measured through the seating the app actually uses, not through the fixed
 * first-position map it used to use. Scoring a line against a fingering
 * nobody plays is how a tier drifts away from the thing it describes.
 */
function measuredTier(version: CelloSongScore, level: ArrangementLevel) {
  const seated = seatLine(
    version.notes.map((note) => ({ midiNumber: note.midiNumber, startTimeMs: note.startTimeMs, durationMs: note.durationMs })),
    { closedFrameOnly: ARRANGEMENT_PROFILES[level].closedFrameOnly },
  );
  return difficultyOf(version.notes, seated).tier;
}

function expectNoteSeated(label: string, level: ArrangementLevel, note: CelloNote): void {
  if (level === 'Beginner' || level === 'Intermediate') expect(note.extension, `${label} extension`).toBe('none');
  expect(NECK_POSITIONS, `${label}/${note.id}`).toContain(note.position);
  // Inside the neck: fourth position anchors at 7 and the little
  // finger reaches 3 above it, one more when extended.
  expect(note.midiNumber - OPEN_STRING_MIDI[note.string], `${label} stop`).toBeLessThanOrEqual(11);
  if (Object.values(OPEN_STRING_MIDI).includes(note.midiNumber)) {
    expect(note.finger, `${label} open string`).toBe('0');
  }
}

/** One arrangement level: no harder than its name, seated in the neck, inside its range, and heard as written. */
function expectLevelVersion(id: string, level: ArrangementLevel, version: CelloSongScore): void {
  const label = `${id}/${level}`;
  const range = ARRANGEMENT_PROFILES[level].range;
  expect(ARRANGEMENT_LEVELS.indexOf(measuredTier(version, level)), `${label} measured difficulty`)
    .toBeLessThanOrEqual(ARRANGEMENT_LEVELS.indexOf(level));
  for (const note of version.notes) expectNoteSeated(label, level, note);
  // The guide is written narrower than the level allows, never wider.
  expect(validateScore(version), label).toEqual([]);
  expect(version.notes.length, label).toBeGreaterThan(0);
  expect(version.notes.every((note) => note.midiNumber >= range.low && note.midiNumber <= range.high), label).toBe(true);

  const cello = soloPartFromScore(version);
  expect(cello.notes.map((note) => note.midiNumber), label).toEqual(version.notes.map((note) => note.midiNumber));
  expect(cello.notes.map((note) => note.startTimeMs), label).toEqual(version.notes.map((note) => note.startTimeMs));
}

function expectPlayableBacking(id: string, backing: BackingTrack | undefined): void {
  expect(backing, id).toBeDefined();
  expect(backing?.parts.length ?? 0, id).toBeGreaterThan(0);
  for (const part of backing?.parts ?? []) {
    expect(part.notes.every((note) => note.startTimeMs >= 0 && note.durationMs > 0), `${id}/${part.id}`).toBe(true);
  }
}
...
describe('bundled scores', () => {
  ...
  it('rejects a bar that does not add up', async () => {
    const { buildScore } = await import('../build');
    expect(() => buildScore({
      id: 'broken',
      timeSignature: [4, 4],
...
describe('library rows', () => {
  ...
  it('validates every compact song and every adaptive arrangement level', async () => {
    ...
    for (const item of COMPACT_SCORES) {
      ...
      expectBeginnerLine(item.id, versions[0]!);
      versions.forEach((version, index) => expectLevelVersion(item.id, ARRANGEMENT_LEVELS[index]!, version));

      expectPlayableBacking(item.id, getBundledBacking(item.id));
    }
  }, 120_000);
